...
class IdentityModule {
  ...
  */
  ...
  decryptMessage(message) {
    ...
    console.info('decryptMessage:message', message);
      ...
      if (funcName === 'deployGUI') {
        ...
        _this.crypto.generateRSAKeyPair().then((keyPair) => {
          ...
          try {
            ...
            console.error('On addGUIListeners from if generateRSAKeyPair method postMessage error: ' + err);
          }
        });
        return;
        ...
        _this.sendGenerateMessage(contents, origin, usernameHint, idpDomain).then((returnedValue) => {
          ...
          try {
            ...
            console.error('On addGUIListeners from if sendGenerateMessage method postMessage error: ' + err);
          }
        ...
        _this.storeIdentity(result, keyPair).then((returnedValue) => {
          ...
          try {
            ...
            console.error('On addGUIListeners from if storeIdentity method postMessage error: ' + err);
          }
  ...
  _getValidToken(hypertyURL) {
    ...
    return new Promise((resolve, reject) => {
      _this.getIdToken(hypertyURL).then(function(identity) {
        ...
        // console.log('[Identity.IdentityModule.getValidToken] Token expires in: ', expiration_date,  ' | time now: ', time_now);
